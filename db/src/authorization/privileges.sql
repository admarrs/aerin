...
-- define the who can access event model data
-- enable RLS on the table holding the data
alter table data.event enable row level security;

-- define the RLS policy controlling what rows are visible to a particular application user
create policy event_access_policy on data.event to api 
using (
...
grant select, insert, update, delete on data.event to api;
grant usage on data.event_id_seq to webuser;
...
grant select, insert, update, delete on api.events to webuser;

-- anonymous users can only request specific columns from this view
grant select (id, title) on api.events to anonymous;
